...
void InitializeApp(char username[]) {
    ...
    while (!stop_app) {
        ...
        switch (current_app_state) {
            case MAIN_MENU: {
                if (IsKeyPressed(KEY_M)) pause_music = !pause_music;
                if (pause_music) PauseMusicStream(app_assets.menu_music_theme);
                else ResumeMusicStream(app_assets.menu_music_theme);
