...
int InitializeApp(char username[])
{
    ...
    SetConfigFlags(FLAG_WINDOW_TOPMOST | FLAG_VSYNC_HINT | FLAG_WINDOW_UNDECORATED);
    SetTargetFPS(60);
        ...
        switch (current_app_state)
        {
            ...
            case GAMEPLAY:
            {
                ...
                if (pause_app && player.health > 0)
                {
                    ...
                    if (show_pause_text)
                        DrawTextEx(app_assets.font, "YOUR GAME IS PAUSED!", (Vector2){75, 300}, 40, 1, GOLD);

                    DrawTextEx(app_assets.font, "PRESS ESC TO CONTINUE", (Vector2){75, 350}, 30, 1, WHITE);
                    DrawTextEx(app_assets.font, "PRESS BACKSPACE TO RETURN TO MENU", (Vector2){75, 375}, 30, 1, WHITE);
                ...
                else if (pause_app && player.health <= 0)
                {
                    ...
                    if (show_game_over_text)
                        DrawTextEx(app_assets.font, "GAME OVER", (Vector2){75, 300}, 40, 1, RED);

                    DrawTextEx(app_assets.font, "PRESS ENTER TO RETURN TO MENU", (Vector2){75, 350}, 30, 1, WHITE);
                    DrawTexture(app_assets.blur, 0, 0, LIGHTGRAY);
                }
                ...
                if ((pause_app == true) && (player.health <= 0) && (IsKeyPressed(KEY_ENTER)))
                {
                    ...
                    player.health = 3;
                    player.score = 0;
                    player.position = (Vector2){683, 600};

                    for (int i = 0; i < MAX_LASERS; i++)
                    {
                        lasers[i].is_active = false;
                        lasers[i].position = (Vector2){lasers[i].position.x, (float)GetScreenHeight() + lasers[i].texture.height};
                    }

                    ResetAsteroidManager(&asteroid_manager);
                }

                if ((pause_app == true) && (player.health > 0) && (IsKeyPressed(KEY_BACKSPACE)))
                {
                    current_app_state = MAIN_MENU;
                    player.health = 3;
                    player.score = 0;
            ...
            case CUTSCENE:
            {
                ...
                DrawTexture(
                    cutscene_assets.images[image_index],
