...
typedef struct AppAssets
{
    ...
    Texture2D numbers[4];       // Números para mostrar a quantidade de vida
    ...
    Sound gameover_sound;       // Som do game over
    Sound hit_sound;            // Som de impacto ou dano
...
AppAssets LoadAppAssets()
{
    ...
    // Carrega as texturas dos números de 0 a 3
    for (int i = 0; i < 4; i++)
    {
...
// Função para atualizar a posição do fundo do jogo, criando um efeito de rolagem vertical
...
{
    // Inicializa posições do fundo caso ainda não tenha sido inicializado
    ...
    // Atualiza a posição dos dois fundos para o efeito de scroll contínuo
    background_y0 += BG_SCROLL_SPEED * GetFrameTime();
    background_y1 += BG_SCROLL_SPEED * GetFrameTime();

    // Se o fundo sair da tela, reposiciona ele para continuar o loop do scroll
...
// Estrutura para armazenar os recursos de uma cutscene (imagens e música)
typedef struct CutsceneAssets
{
    Texture2D images[3]; // Array de 3 imagens usadas na cutscene
    Music music;         // Música da cutscene
} CutsceneAssets;

// Função que carrega as imagens e música da cutscene
CutsceneAssets LoadCutsceneAssets()
{
    CutsceneAssets cutscene_assets;

    // Carrega as 3 imagens da cutscene
    for (int i = 0; i < 3; i++)
    {
        cutscene_assets.images[i] = LoadTexture(TextFormat("assets/cutscene/cutscene_%d.png", i));
    }

    // Carrega a música da cutscene
    cutscene_assets.music = LoadMusicStream("assets/sounds/cutscene_audio.mp3");

    return cutscene_assets;
}

// Estrutura para armazenar as texturas dos comandos exibidos na tela de ajuda
